...
static void handle_transform(TSCont contp){
  ...
  if (!data) {
    ...
    TSDebug(PLUGIN_NAME, "\tWriting % bytes on VConn", TSVIONBytesGet(input_vio));
  ...
  TSDebug(PLUGIN_NAME, "\ttoWrite is %", towrite);
  ...
  if (towrite > 0) {
    ...
    TSDebug(PLUGIN_NAME, "\tavail is %", avail);
    if (towrite > avail) {
      towrite = avail;
    }
...
static int null_transform(TSCont contp, TSEvent event, void *edata){
